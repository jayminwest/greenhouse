...
**Argument:** `$ARGUMENTS` — optional: a label or area to focus on (e.g., `cli`, `daemon`, `dispatcher`). If empty, analyze everything.
...
### c. Complexity
- Is the change localized or does it cut across multiple subsystems (CLI, daemon, poller, dispatcher)?
